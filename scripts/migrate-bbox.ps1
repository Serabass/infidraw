...
Write-Host "Migrating bbox fields for existing strokes..." -ForegroundColor Cyan
...
# SQL для обновления bbox полей из JSONB данных
$sql = @"
...
CREATE INDEX IF NOT EXISTS idx_stroke_bbox ON stroke_events USING GIST (
...
$result = docker-compose exec -T postgres psql -U $dbUser -d $dbName -c $sql 2>&1
